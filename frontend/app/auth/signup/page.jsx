...
export default function SignupPage(){
  ...
  const handle = async (e)=>{ e.preventDefault(); const res = await signup({email,password}); if (res.token) router.push('/'); else setErr(res.message || 'Signup failed'); };
      ...
      <div className="max-w-md mx-auto p-6 rounded-lg border bg-white dark:bg-gray-800">
        ...
        <form onSubmit={handle} className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            <button type="submit" className="px-4 py-2 bg-amber-600 text-white rounded">Create account</button>
